...
from screens.SignupEnterNameScreen import SignupEnterNameScreen
from screens.SignupEmailScreen import SignupEmailScreen
...
@when("I enter the phone number \"{phone_number}\"")
...
@when("I tap \"Continue\" on phone number screen")
...
@then("I enter the \"code\" on code validation screen")
...
@then("I enter my email address \"{email_address}\"")
def enter_email_address(context, email_address):
    with create_james_driver() as context.james_driver:
        email_address_screen = SignupEmailScreen(context.james_driver)
        email_address_screen.enter_email_address(email_address)
        email_address_screen.tap_continue()


@then("I tap continue on email address screen")
def email_address_tap_continue(context):
    with create_james_driver() as context.james_driver:
        email_address_screen = SignupEmailScreen(context.james_driver)
        email_address_screen.tap_continue()
...
def enter_user_names(context):
    with create_james_driver() as context.james_driver:
        enter_name_screen = SignupEnterNameScreen(context.james_driver)
        enter_name_screen.enter_first_name("Dario")
        enter_name_screen.enter_last_name("Mendoza")


@then("I tap continue on enter name screen")
def enter_user_names_tap_continue(context):
    with create_james_driver() as context.james_driver:
        enter_name_screen = SignupEnterNameScreen(context.james_driver)
        enter_name_screen.tap_continue()
